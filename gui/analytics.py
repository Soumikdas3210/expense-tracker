...
class AnalyticsFrame(tk.Frame):
    def __init__(self, parent, app):
        ...
        self.build_trend_view_selector()
    ...
    def build_trend_view_selector(self):
        selector_frame = tk.Frame(self)
        selector_frame.pack(padx=10, pady=(0, 5), fill="x")

        selector_label = tk.Label(selector_frame, text="Bar / Line Chart View:")
        selector_label.pack(side="left")

        self.trend_view_dropdown = ttk.Combobox(selector_frame, values=["Monthly", "Daily (This Month)"], state="readonly", width=20)
        self.trend_view_dropdown.set("Monthly")
        self.trend_view_dropdown.pack(side="left", padx=5)
        self.trend_view_dropdown.bind("<<ComboboxSelected>>", self.on_trend_view_changed)

    def on_trend_view_changed(self, event):
        self.load_stats()
    ...
    def get_trend_totals(self, expenses):
        view = self.trend_view_dropdown.get()

        if view == "Daily (This Month)":
            ...
            return analytics.spending_trend(filtered_expenses, "daily")
        else:
            return analytics.spending_trend(expenses, "monthly")

    def get_empty_chart_message(self):
        view = self.trend_view_dropdown.get()

        if view == "Daily (This Month)":
            return "No expense data for this month yet"
        else:
    ...
    def draw_bar_chart(self, expenses):
        ...
        totals = self.get_trend_totals(expenses)
        ...
        if len(sorted_keys) == 0:
            self.bar_axes.text(0.5, 0.5, self.get_empty_chart_message(), ha="center")
        else:
            self.bar_axes.bar(sorted_keys, values)
            self.bar_axes.tick_params(axis="x", rotation=45)

        view = self.trend_view_dropdown.get()
        if view == "Daily (This Month)":
            self.bar_axes.set_title("Daily Spending (This Month)")
        else:
    ...
    def draw_line_chart(self, expenses):
        ...
        if len(sorted_keys) == 0:
            self.line_axes.text(0.5, 0.5, self.get_empty_chart_message(), ha="center")
        else:
            self.line_axes.plot(sorted_keys, values, marker="o")
            self.line_axes.tick_params(axis="x", rotation=45)

        view = self.trend_view_dropdown.get()
        if view == "Daily (This Month)":
            self.line_axes.set_title("Daily Spending Trend (This Month)")
        else:
